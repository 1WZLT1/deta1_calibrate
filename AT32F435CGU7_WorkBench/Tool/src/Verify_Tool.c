...
#define FW_SIZE 0x70000
#define FW_BLOCK 0

const __attribute__((used,at(0x08000000 + 0x10000))) Firmware_Data_t Firmware = {
	.voctor = {0},
	.Hardware_Version = 0x0001060000,
	.Firmware_Size = FW_SIZE,
	.Firmware_Block = FW_BLOCK,
  .Firmware_Version = 1,
	.Strings_Size = 0,
};
...
static const uint32_t CRC24QTable[]={
...
uint32_t CRC24Q_Table(const uint8_t *buff, int len)
{
    ...
    for (int i = 0;i < len;i++) 
	{
		crc=((crc << 8) & 0xFFFFFF) ^ CRC24QTable[(crc >> 16) ^ buff[i]];
	}
    return crc;
}
