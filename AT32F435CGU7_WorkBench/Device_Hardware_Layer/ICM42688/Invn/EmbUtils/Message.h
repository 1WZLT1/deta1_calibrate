...
#include "ICM42688/Invn/InvExport.h"
...
#endif
extern void myPrintf(void *s,int len);
#include "stdio.h"
 ...
 */
//#define INV_MSG(level, format,arg...) do{uint8_t __buffer[64];int __length = sprintf((char*)__buffer,format,##arg);myPrintf(__buffer, __length);}while(0)
#define INV_MSG(level, format,arg...) //do{uint8_t __buffer[64];int __length = sprintf((char*)__buffer,format,##arg);myPrintf(__buffer, __length);}while(0)
