...
#include "../../Device_Hardware_Layer/ICM42688/Invn/Drivers/Icm426xx/Icm426xxDriver_HL_apex.h"
...
int inv_icm426xx_init(struct inv_icm426xx * s, struct inv_icm426xx_serif * serif, void (*sensor_event_cb)(inv_icm426xx_sensor_event_t * event))
{
	...
	if((status |= inv_icm426xx_configure_serial_interface(s)) != 0 )//设置42688模式
		return status;
	 ...
	 */
	s->sensor_event_cb = sensor_event_cb;//设置回调
...
int inv_icm426xx_device_reset(struct inv_icm426xx * s)
{
	...
	/* Set memory bank 1 */
	status |= inv_icm426xx_set_reg_bank(s, 1);/*切换到bank1*/
	/* save registers necessary to perform soft reset while still keeping communication link alive */
	status |= inv_icm426xx_read_reg(s, MPUREG_INTF_CONFIG4_B1, 1, &intf_cfg4_reg); // intf_cfg4_reg 返回的是当前硬件状态
	status |= inv_icm426xx_read_reg(s, MPUREG_INTF_CONFIG6_B1, 1, &intf_cfg6_reg); // intf_cfg6_reg 5f默认值返回
	/* Set memory bank 0 */
	...
	status |= inv_icm426xx_write_reg(s, MPUREG_CHIP_CONFIG, 1, &data);//软复位
	if(status)
		return status;
	...
	do {
		...
		status |= inv_icm426xx_write_reg(s, MPUREG_INTF_CONFIG4_B1, 1, &intf_cfg4_reg);/*设置硬件状态*/
		status |= inv_icm426xx_read_reg(s, MPUREG_OTP_SEC_STATUS_B1, 1, &data);//不知道有啥作用但是能过下面while循环问题不大
		status |= inv_icm426xx_set_reg_bank(s, 0);
	...
	status |= inv_icm426xx_read_reg(s, MPUREG_INT_STATUS, 1, &data);//读取当前传感器中断状态
	if (0 == (data & BIT_INT_STATUS_RESET_DONE))//软件重置完成 为 1
	{
		return INV_ERROR_UNEXPECTED;//软件重置未完成
	}

	status |= inv_icm426xx_set_reg_bank(s, 1);
	status |= inv_icm426xx_write_reg(s, MPUREG_INTF_CONFIG6_B1, 1, &intf_cfg6_reg);//默认值写入
	
	
//	/* Configure FSYNC on INT2=pin 9 */
//	status |= inv_icm426xx_read_reg(s, MPUREG_INTF_CONFIG5_B1, 1, &data);//读pin9(INT2)的模式
//	data &= (uint8_t)~BIT_INTF_CONFIG5_GPIO_PAD_SEL_MASK;//中断输出
//	
//	//data |= (1 << BIT_INTF_CONFIG5_GPIO_PAD_SEL_POS)	;//设置pin9为帧同步SYNC
//
//	
//	status |= inv_icm426xx_write_reg(s, MPUREG_INTF_CONFIG5_B1, 1, &data);//写Pin9
	
	status |= inv_icm426xx_set_reg_bank(s, 0);

	/* Load hardware POR values in register cache */
	status |= inv_icm426xx_read_reg(s, MPUREG_INTF_CONFIG1, 4, &s->register_cache.intf_cfg_1_reg);//获得0x4d往后4个及储存期的值

	status |= inv_icm426xx_read_reg(s, MPUREG_TMST_CONFIG, 1, &s->register_cache.tmst_cfg_reg);//读取时间戳相关内容

	/* Read and set endianess for further processing */
	status |= inv_icm426xx_read_reg(s, MPUREG_INTF_CONFIG0, 1, &data);//fifo按照大端报告
...
int inv_icm426xx_enable_accel_low_noise_mode(struct inv_icm426xx * s)
{
	...
	if(accel_mode == ICM426XX_PWR_MGMT_0_ACCEL_MODE_OFF && gyro_mode == ICM426XX_PWR_MGMT_0_GYRO_MODE_OFF) {
		...
		if(s->fifo_is_used) {
			status |= inv_icm426xx_read_reg(s, MPUREG_FIFO_CONFIG1, 1, &data);//0x2c
			data |= (uint8_t)ICM426XX_FIFO_CONFIG1_ACCEL_EN;//acc 加到 fifo
			data |= (uint8_t)ICM426XX_FIFO_CONFIG1_GYRO_EN;//gyro 加到 fifo
			if(s->fifo_highres_enabled)
				data |= (uint8_t)ICM426XX_FIFO_CONFIG1_HIRES_EN;
			status |= inv_icm426xx_write_reg(s, MPUREG_FIFO_CONFIG1, 1, &data);
		}
		/* Read data endianess in order to process correctly data */
		status |= inv_icm426xx_read_reg(s, MPUREG_INTF_CONFIG0, 1, &data_endianess);
		s->endianess_data = data_endianess & BIT_DATA_ENDIAN_MASK;//传感器数据大小端判断
	}
#endif
	...
	status |= inv_icm426xx_write_reg(s, MPUREG_ACCEL_GYRO_CONFIG0, 1, &data);//默认滤波设置
	...
	status |= inv_icm426xx_write_reg(s, MPUREG_PWR_MGMT_0, 1, &s->register_cache.pwr_mngt_0_reg);//将加表设置为低噪声模式
	inv_icm426xx_sleep_us(200);
	...
	if (accel_mode == ICM426XX_PWR_MGMT_0_ACCEL_MODE_OFF) {
		 ...
		 */
		if(s->fifo_is_used) {
			s->accel_start_time_us = inv_icm426xx_get_time_us();//获得时间戳
		}
	}
#endif
...
int inv_icm426xx_enable_gyro_low_noise_mode(struct inv_icm426xx * s)
{
	...
	status |= inv_icm426xx_write_reg(s, MPUREG_ACCEL_GYRO_CONFIG0, 1, &data);//默认值写入
	...
	status |= inv_icm426xx_write_reg(s, MPUREG_PWR_MGMT_0, 1, &s->register_cache.pwr_mngt_0_reg);//开启陀螺低噪声
	inv_icm426xx_sleep_us(200);
	...
	if (gyro_mode == ICM426XX_PWR_MGMT_0_GYRO_MODE_OFF) {
		 ...
		 */
		if(s->fifo_is_used) {
			s->gyro_start_time_us = inv_icm426xx_get_time_us();//获得时间戳
		}
	}
#endif
...
int inv_icm426xx_get_config_int1(struct inv_icm426xx * s, inv_icm426xx_interrupt_parameter_t * interrupt_to_configure)
{
	...
	status |= inv_icm426xx_read_reg(s, MPUREG_INT_SOURCE0, 2, data); //当芯片内部的软复位（Soft Reset）流程彻底完成、寄存器恢复默认值并准备好接受新配置时，通过物理引脚 INT1 向主控芯片（MCU）发送一个信号。
																																	 //运动模式|抬腕|i3c错误
	status |= inv_icm426xx_set_reg_bank(s, 4);      
	status |= inv_icm426xx_read_reg(s, MPUREG_INT_SOURCE6_B4, 1, &data[2]); /* 获得中断状态 */
	status |= inv_icm426xx_set_reg_bank(s, 0);
	...
	interrupt_to_configure->INV_ICM426XX_FIFO_FULL = (inv_icm426xx_interrupt_value) ((data[0] & BIT_INT_SOURCE0_FIFO_FULL_INT1_EN) >> BIT_INT_FIFO_FULL_INT_EN_POS);/*表示当前int1中断开启开关*/
	...
	interrupt_to_configure->INV_ICM426XX_WOM_Z = (inv_icm426xx_interrupt_value) ((data[1] & BIT_INT_SOURCE1_WOM_Z_INT1_EN ) >> BIT_INT_WOM_Z_INT_EN_POS);/*表示当前int1中断开启开关*/
...
int inv_icm426xx_set_config_int1(struct inv_icm426xx * s, inv_icm426xx_interrupt_parameter_t * interrupt_to_configure)
{
	...
	status |= inv_icm426xx_write_reg(s, MPUREG_INT_SOURCE6_B4, 1, &data[2]);
	status |= inv_icm426xx_set_reg_bank(s, 0);
	status |= inv_icm426xx_write_reg(s, MPUREG_INT_SOURCE0, 2, data); //唯一开启也就只是走step开启一次中断  
...
int inv_icm426xx_get_data_from_registers(struct inv_icm426xx * s)
{
	...
	if(int_status & BIT_INT_STATUS_DRDY) {
		 ...
		 */
		if((s->serif.serif_type == ICM426XX_UI_I2C) || (s->serif.serif_type == ICM426XX_UI_I3C))
...
int inv_icm426xx_set_accel_frequency(struct inv_icm426xx * s, const ICM426XX_ACCEL_CONFIG0_ODR_t frequency)
{
	 ...
	 */
	ICM426XX_PWR_MGMT_0_ACCEL_MODE_t accel_pwr_mode = (ICM426XX_PWR_MGMT_0_ACCEL_MODE_t)(s->register_cache.pwr_mngt_0_reg & BIT_PWR_MGMT_0_ACCEL_MODE_MASK);//bank0 0x4e读取 判断将加速度计置于低噪声(LN)模式
	if(ICM426XX_PWR_MGMT_0_ACCEL_MODE_LP != accel_pwr_mode)//在低功耗模式下
	...
	return inv_icm426xx_write_reg(s, MPUREG_ACCEL_CONFIG0, 1, &s->register_cache.accel_cfg_0_reg);//1k 8g
}
...
int inv_icm426xx_set_gyro_fsr(struct inv_icm426xx * s, ICM426XX_GYRO_CONFIG0_FS_SEL_t gyro_fsr_dps)
{
	...
	return inv_icm426xx_write_reg(s, MPUREG_GYRO_CONFIG0, 1, &s->register_cache.gyro_cfg_0_reg);//设置输出频率
}
...
/* CLKIN is not supported for all parts, prevent the CLKIN to be enabled in this case. */
		...
		status |= inv_icm426xx_set_reg_bank(s, 1);//切换到bank 1
		status |= inv_icm426xx_read_reg(s, MPUREG_INTF_CONFIG5_B1, 1, &data);//设置int2
		data &= (uint8_t)~BIT_INTF_CONFIG5_GPIO_PAD_SEL_MASK;
		data |= (2 << BIT_INTF_CONFIG5_GPIO_PAD_SEL_POS);
		status |= inv_icm426xx_write_reg(s, MPUREG_INTF_CONFIG5_B1, 1, &data);//设置成外部时钟输入
		status |= inv_icm426xx_set_reg_bank(s, 0);
		s->register_cache.intf_cfg_1_reg = 0x95;
		status |= inv_icm426xx_write_reg(s, MPUREG_INTF_CONFIG1, 1, &s->register_cache.intf_cfg_1_reg);
		//设置如果有Pll就用Pll 否则用内部振荡器
		//需要rtc时钟输入
		status |= inv_icm426xx_configure_timestamp_resolution(s, ICM426XX_TMST_CONFIG_RESOL_16us);//使能时间戳寄存器 | 16us间隔
#else
		(void)s;
		(void)enable;
		return INV_ERROR;
...
int inv_icm426xx_configure_fifo(struct inv_icm426xx * s, INV_ICM426XX_FIFO_CONFIG_t fifo_config)
{
	...
	switch (fifo_config) {
		...
		case INV_ICM426XX_FIFO_ENABLED :
			...
			*/
			status |= inv_icm426xx_read_reg(s, MPUREG_INTF_CONFIG0, 1, &data);//默认大端
			data |= (uint8_t)ICM426XX_INTF_CONFIG0_FIFO_COUNT_REC_RECORD;
			data &= (uint8_t)~BIT_FIFO_COUNT_ENDIAN_MASK; // little endian
			status |= inv_icm426xx_write_reg(s, MPUREG_INTF_CONFIG0, 1, &data);//fifo小端 然后 fifo位数20位
			data = (uint8_t)ICM426XX_FIFO_CONFIG_MODE_SNAPSHOT;
			status |= inv_icm426xx_write_reg(s, MPUREG_FIFO_CONFIG, 1, &data);//完全停止
			s->register_cache.tmst_cfg_reg |= ICM426XX_TMST_CONFIG_TMST_EN;
			
#if (!INV_ICM426XX_LIGHTWEIGHT_DRIVER)
			/* restart and reset FIFO configuration */
			status |= inv_icm426xx_read_reg(s, MPUREG_FIFO_CONFIG1, 1, &data);
			data &= (uint8_t)~(BIT_FIFO_CONFIG1_GYRO_MASK | BIT_FIFO_CONFIG1_ACCEL_MASK);//获得默认值
			data |= (BIT_FIFO_CONFIG1_TEMP_MASK | BIT_FIFO_CONFIG1_TMST_FSYNC_MASK);//温度进入temp fifo存储fsync
#else
			status |= inv_icm426xx_read_reg(s, MPUREG_FIFO_CONFIG1, 1, &data);
			...
			data |= (uint8_t)ICM426XX_FIFO_CONFIG1_WM_GT_TH_EN;//FIFO 里的字节数够不够 FIFO_WM_TH 够了产生一次中断
			status |= inv_icm426xx_write_reg(s, MPUREG_FIFO_CONFIG1, 1, &data);
			/* Configure FIFO WM so that INT is triggered for each packet */
			data = 0x1;
			status |= inv_icm426xx_write_reg(s, MPUREG_FIFO_CONFIG2, 1, &data);//你不提取（读取）数据，中断就不会再次触发。

			/* Disable Data Ready Interrupt */
			status |= inv_icm426xx_get_config_int1(s, &config_int);//获得int1的中断配置状态
			config_int.INV_ICM426XX_UI_DRDY = INV_ICM426XX_DISABLE;
			status |= inv_icm426xx_set_config_int1(s, &config_int);//设置int1中断没有配置啥无用
			break;
...
static int inv_icm426xx_configure_serial_interface(struct inv_icm426xx * s)
{
	...
	switch(s->serif.serif_type) {
		...
		case ICM426XX_UI_SPI4:
			...
			status |= inv_icm426xx_write_reg(s, MPUREG_INTF_CONFIG4_B1, 1, &value);//采用4线spi模式 value = 0x02
			break;
	...
	/* Set memory bank 0 */
	status |= inv_icm426xx_set_reg_bank(s, 0);//切换到bank0
...
static int inv_icm426xx_init_hardware_from_ui(struct inv_icm426xx * s)
{
	...
	inv_icm426xx_interrupt_parameter_t config_int = {
		...
		.INV_ICM426XX_FIFO_THS      = INV_ICM426XX_DISABLE,	/*fifo存满产生中断*/
		.INV_ICM426XX_FIFO_FULL     = INV_ICM426XX_DISABLE,	
		.INV_ICM426XX_SMD           = INV_ICM426XX_DISABLE,	/*显著运动开启用作检测时间段内是否出现运动*/  
		.INV_ICM426XX_WOM_X         = INV_ICM426XX_DISABLE,	/*WOM阈值检测 让传感器在检测到多大的动作时发送中断信号*/  
		.INV_ICM426XX_WOM_Y         = INV_ICM426XX_DISABLE,	
		.INV_ICM426XX_WOM_Z         = INV_ICM426XX_DISABLE,	  
		.INV_ICM426XX_STEP_DET      = INV_ICM426XX_DISABLE,	/*步进检测*/  
		.INV_ICM426XX_STEP_CNT_OVFL = INV_ICM426XX_DISABLE,  /*溢出中断*/	  
		.INV_ICM426XX_TILT_DET      = INV_ICM426XX_DISABLE,  /*倾角中断*/
		.INV_ICM426XX_SLEEP_DET     = INV_ICM426XX_DISABLE,	/*休眠检测*/
		.INV_ICM426XX_WAKE_DET      = INV_ICM426XX_DISABLE,	/*唤醒检测*/
		.INV_ICM426XX_TAP_DET       = INV_ICM426XX_DISABLE,	/*点击读取*/
    ...
    status |= inv_icm426xx_device_reset(s);
		...
		/* 配置为CLKIN */
		status |= inv_icm426xx_enable_clkin_rtc(s, 1);
	
    /* 配置 Gyro：±2000dps */
    s->register_cache.gyro_cfg_0_reg &= (uint8_t)~BIT_GYRO_CONFIG0_FS_SEL_MASK;
    s->register_cache.gyro_cfg_0_reg |= (uint8_t)ICM426XX_GYRO_CONFIG0_FS_SEL_2000dps;
    status |= inv_icm426xx_write_reg(s, MPUREG_GYRO_CONFIG0, 1,
                                     &s->register_cache.gyro_cfg_0_reg);

    /* 配置 Accel：你如果要 ±8g，就用 8g 宏 */
    s->register_cache.accel_cfg_0_reg &= (uint8_t)~BIT_ACCEL_CONFIG0_FS_SEL_MASK;
    s->register_cache.accel_cfg_0_reg |= (uint8_t)ICM426XX_ACCEL_CONFIG0_FS_SEL_8g;
    status |= inv_icm426xx_write_reg(s, MPUREG_ACCEL_CONFIG0, 1,
                                     &s->register_cache.accel_cfg_0_reg);

    /* 关闭 FIFO，使用寄存器实时读取 */
    data = ICM426XX_FIFO_CONFIG_MODE_BYPASS;
    status |= inv_icm426xx_write_reg(s, MPUREG_FIFO_CONFIG, 1, &data);
    status |= inv_icm426xx_configure_fifo(s, INV_ICM426XX_FIFO_DISABLED);
    ...
    /* timestamp 分辨率 16us */
    status |= inv_icm426xx_configure_timestamp_resolution(
        s,
    ...
    /* DRDY 清除方式：读数据寄存器后清除 */
    status |= inv_icm426xx_read_reg(s, MPUREG_INT_CONFIG0, 1, &data);
    data &= (uint8_t)~(0x03 << 4);
    data |=  (uint8_t)(0x01 << 4);
    status |= inv_icm426xx_write_reg(s, MPUREG_INT_CONFIG0, 1, &data);
    ...
    /* 默认带宽设置，低延迟可优先 ODR/2，没有宏就用 ODR/4 */
